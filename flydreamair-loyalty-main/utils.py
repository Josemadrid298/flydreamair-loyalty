...
def save_users(users) -> bool:
    # return success so the UI can show a real error instead of a fake success
...
def calculate_points(base_amount: float, tier: str) -> int:
    ...
    # round (not int) so 51 * 1.5 -> 77, not 76
...
def add_transaction(user_data: dict, description: str, points: int):
    ...
    # full bucket check so tier downgrades when points fall below a threshold
